  ...
  .directive('pwCanvas', function() {
	return {
	  ...
	  link: function postLink(scope, elm) {
		...
		options.width = scope.width || 400;
		options.height = scope.height || 300;
		...
		scope.$watch('options.imageSrc', function() {
		  var image = new Image();
		  image.onload = function() {
			  ctx.drawImage(image, 0, 0);
		  };
		  image.src = scope.options.imageSrc;
		...
		ctxTmp.lineWidth = 10;
		ctxTmp.strokeStyle = scope.color;
		...
		scope.$watch('lineWidth', function(newValue) {
		  ...
		  if (newValue && typeof newValue === 'number') {
			ctxTmp.lineWidth = scope.lineWidth = newValue;
		  }
		});
